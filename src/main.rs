...
fn getAdvancementItems() -> Vec<Item> {
  let mut res = Vec::new();
  for _ in 0..4 { res.push(Item::ProgressiveSword); }
  // @drift: the web code randomly chooses a specific bottle each seed
...
fn main() {
  let advancement_items = getAdvancementItems();
  println!("advancement_items: {:?}", advancement_items);

  let nice_items = getNiceItems();
  println!("nice_items: {:?}", nice_items);

  let junk_items = getItemPool();
  println!("item_pool: {:?}", junk_items);

  let dungeon_items = getDungeonPool();
